...
const Products = () => {
    ...
    const filterData = (filters, productList) => {
        ...
        if (!filters.stock) {
            filteredList = filteredList.filter((ele) => ele.stock > 100);
        }
        if (!filters.expiryDate) {
            let currentDate = new Date();
            filteredList = filteredList.filter((ele) => new Date(ele.expiryDate) > currentDate);
        }
...
<Navbar/>
    {loading&&<Loader/>}   
    {!loading && productList.length>0 &&<div className="product_container">
        ...
        <div className="inner-wrapper">
            <div className="product-row">
                ...
                <div className="product-option">
                    ...
                    <label className="product-checkbox"><input type="checkbox" name="expiryDate" id="expiredCheckBox" onChange={handleInput} checked={filter.expiryDate} />Expired</label>
                    <label className="product-checkbox"><input type="checkbox" name="stock" id="lowStockCheckBox" onChange={handleInput} checked={filter.stock} />Low Stock</label>
